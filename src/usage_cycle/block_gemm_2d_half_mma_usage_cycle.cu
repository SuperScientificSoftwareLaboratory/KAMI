...
#pragma unroll
                ...
                comm_start_cycle = clock();
                for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; i++)
                {
                    for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK; j++)
                    {

                        a_frags_compute[i][j][0] = a_compute[i][j][lane_id * 4 + 0];
                        a_frags_compute[i][j][1] = a_compute[i][j][lane_id * 4 + 1];
                        a_frags_compute[i][j][2] = a_compute[i][j][lane_id * 4 + 2];
                        a_frags_compute[i][j][3] = a_compute[i][j][lane_id * 4 + 3];
                    }
                }
                ...
                for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; i++)
                {
                    for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK; j++)
                    {
                        ...
                        b_frags_compute[i][j][0] = b_compute[i][j][lane_id * 2 + 0];
                        b_frags_compute[i][j][1] = b_compute[i][j][lane_id * 2 + 1];
                    }
                }
...
#pragma unroll
    ...
    if (lane_id == 0)
    {
        printf("warp_id: 2d,%d,%d,%d\n", warp_id, comm_total_cycles / (NUM_ITER * NUM_PIPE_BLOCK), comp_total_cycles / (NUM_ITER * NUM_PIPE_BLOCK));
    }
    __syncthreads();
