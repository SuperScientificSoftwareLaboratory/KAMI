...
__global__ void block_gemm_1d_half_mma(const half *A, const half *B, half *C, const half alpha, const half beta)
{
  ...
  uint32_t b_frags_compute[SUB_N_BLOCK / WMMA_N_BLOCK][K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK][2];
      ...
      for (int idx_pipe = 0; idx_pipe < NUM_PIPE_BLOCK; ++idx_pipe)
      {
        ...
        if (warp_id == idx_stage)
        {
          ...
          for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; i++)
          {
            for (int j = 0; j < K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK; j++)
            {
              ...
              uint32_t *b1 = (uint32_t *)(shmem_b + lane_id * 4 + (i * (K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK);
              uint32_t *b2 = (uint32_t *)(shmem_b + lane_id * 4 + (i * (K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK + 2);

              *b1 = b_frags[i][real_j][0];
              *b2 = b_frags[i][real_j][1];
            }
          }
        }
        ...
        for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; i++)
        {
          for (int j = 0; j < K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK; j++)
          {
            ...
            unsigned int comm_comm_start_cycle = clock();
            b_frags_compute[i][j][0] = *b1;
            b_frags_compute[i][j][1] = *b2;
            comm_total_cycles += clock() - comm_comm_start_cycle;
          }
        }
        ...
        for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
        {
          for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; ++j)
          {
            for (int k = 0; k < K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK; ++k)
            {
              ...
              int a = j + idx_stage * (SUB_N_BLOCK / WMMA_N_BLOCK);

              mma_m16n8k16_fp16(c_frags[i][a], a_frags[i][real_k], b_frags_compute[j][k]);
            }
          }
        }
