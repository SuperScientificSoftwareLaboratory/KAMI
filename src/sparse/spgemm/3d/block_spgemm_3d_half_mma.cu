...
#define SUB_BSR_K_BLOCK (BSR_K_BLOCK / NUM_RANK_BLOCK / NUM_RANK_BLOCK)

const int shmem_size_block_val = ((SUB_M_BLOCK * SUB_K_BLOCK * NUM_RANK_BLOCK * NUM_RANK_BLOCK) + (SUB_K_BLOCK * SUB_N_BLOCK * NUM_RANK_BLOCK * NUM_RANK_BLOCK)) * sizeof(half);
const int shmem_size_block_row_ptr = ((SUB_BSR_M_BLOCK + 1) + (SUB_BSR_K_BLOCK + 1)) * NUM_RANK_BLOCK * NUM_RANK_BLOCK * sizeof(int);
const int shmem_size_block_col_idx = (SUB_BSR_M_BLOCK * SUB_BSR_K_BLOCK + SUB_BSR_K_BLOCK * SUB_BSR_N_BLOCK) * NUM_RANK_BLOCK * NUM_RANK_BLOCK * sizeof(int);
...
__global__ void block_spgemm_3d_half_mma(const int *RowPtrA, const int *ColIdxA, const half *ValA,
                                         ...
                                         const half alpha, const half beta)
{
    ...
    int *shmem_b_row_ptr = shmem_a_row_ptr + (SUB_BSR_M_BLOCK + 1) * NUM_RANK_BLOCK * NUM_RANK_BLOCK;
    int *shmem_a_col_idx = shmem_b_row_ptr + (SUB_BSR_K_BLOCK + 1) * NUM_RANK_BLOCK * NUM_RANK_BLOCK;
    int *shmem_b_col_idx = shmem_a_col_idx + (SUB_BSR_M_BLOCK * SUB_BSR_K_BLOCK) * NUM_RANK_BLOCK * NUM_RANK_BLOCK;
...

    {
        ...
        int col_start_bsr = warp_col * SUB_BSR_K_BLOCK * NUM_RANK_BLOCK + warp_dep * SUB_BSR_K_BLOCK;
    ...
    {
        int row_start_bsr = warp_row * SUB_BSR_K_BLOCK * NUM_RANK_BLOCK + warp_dep * SUB_BSR_K_BLOCK;
    ...
    for (int idx_iter = 0; idx_iter < NUM_ITER; idx_iter++)
        for (int idx_stage = 0; idx_stage < NUM_RANK_BLOCK; idx_stage++)
        {
            ...
            {
                int dep_offset = warp_dep * SUB_M_BLOCK * SUB_K_BLOCK * NUM_RANK_BLOCK;
                for (int i = 0; i < SUB_BSR_M_BLOCK; i++)
                {
                    for (int j = 0; j < SUB_BSR_K_BLOCK; j++)
                    {
                        ...
                        for (int k = 0; k < BSR_M / WMMA_M_BLOCK; k++)
                        {
                            ...
                            {
                                *(uint32_t *)(shmem_a + shmem_offset + lane_id * 8 + 0 + (k * (BSR_K / WMMA_K_BLOCK) + l) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * (SUB_M_BLOCK * SUB_K_BLOCK) + dep_offset) = a_frags[i * SUB_BSR_K_BLOCK + j][k][l][0];
                                *(uint32_t *)(shmem_a + shmem_offset + lane_id * 8 + 2 + (k * (BSR_K / WMMA_K_BLOCK) + l) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * (SUB_M_BLOCK * SUB_K_BLOCK) + dep_offset) = a_frags[i * SUB_BSR_K_BLOCK + j][k][l][1];
                                *(uint32_t *)(shmem_a + shmem_offset + lane_id * 8 + 4 + (k * (BSR_K / WMMA_K_BLOCK) + l) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * (SUB_M_BLOCK * SUB_K_BLOCK) + dep_offset) = a_frags[i * SUB_BSR_K_BLOCK + j][k][l][2];
                                *(uint32_t *)(shmem_a + shmem_offset + lane_id * 8 + 6 + (k * (BSR_K / WMMA_K_BLOCK) + l) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * (SUB_M_BLOCK * SUB_K_BLOCK) + dep_offset) = a_frags[i * SUB_BSR_K_BLOCK + j][k][l][3];
                            }
                        }
                    }
                }
                ...
                {
                    shmem_a_row_ptr[warp_row * (SUB_BSR_M_BLOCK + 1) + i + warp_dep * (SUB_BSR_M_BLOCK + 1) * NUM_RANK_BLOCK] = row_ptr_a_local[i];
                }

                for (int i = 0; i < SUB_BSR_M_BLOCK * SUB_BSR_K_BLOCK; i++)
                {
                    shmem_a_col_idx[warp_row * (SUB_BSR_M_BLOCK * SUB_BSR_K_BLOCK) + i + warp_dep * (SUB_BSR_M_BLOCK * SUB_BSR_K_BLOCK) * NUM_RANK_BLOCK] = col_idx_a_local[i];
                }
            }

            if (warp_row == idx_stage)
            {
                int dep_offset = warp_dep * SUB_K_BLOCK * SUB_N_BLOCK * NUM_RANK_BLOCK;
                for (int i = 0; i < SUB_BSR_K_BLOCK; i++)
                {
                    for (int j = 0; j < SUB_BSR_N_BLOCK; j++)
                    {
                        ...
                        for (int k = 0; k < BSR_K / WMMA_K_BLOCK; k++)
                        {
                            ...
                            {
                                *(uint32_t *)(shmem_b + shmem_offset + lane_id * 4 + 0 + (k * (BSR_N / WMMA_N_BLOCK) + l) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * (SUB_K_BLOCK * SUB_N_BLOCK) + dep_offset) = b_frags[i * SUB_BSR_N_BLOCK + j][k][l][0];
                                *(uint32_t *)(shmem_b + shmem_offset + lane_id * 4 + 2 + (k * (BSR_N / WMMA_N_BLOCK) + l) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * (SUB_K_BLOCK * SUB_N_BLOCK) + dep_offset) = b_frags[i * SUB_BSR_N_BLOCK + j][k][l][1];
                            }
                        }
                    }
                }
                ...
                {
                    shmem_b_row_ptr[warp_col * (SUB_BSR_K_BLOCK + 1) + i + warp_dep * (SUB_BSR_K_BLOCK + 1) * NUM_RANK_BLOCK] = row_ptr_b_local[i];
                }

                for (int i = 0; i < SUB_BSR_K_BLOCK * SUB_BSR_N_BLOCK; i++)
                {
                    shmem_b_col_idx[warp_col * (SUB_BSR_K_BLOCK * SUB_BSR_N_BLOCK) + i + warp_dep * (SUB_BSR_K_BLOCK * SUB_BSR_N_BLOCK) * NUM_RANK_BLOCK] = col_idx_b_local[i];
                }
            }
            ...
            for (int i = 0; i < SUB_BSR_M_BLOCK; i++)
            {
                for (int j = 0; j < SUB_BSR_K_BLOCK; j++)
                {
                    ...
                    for (int k = 0; k < BSR_M / WMMA_M_BLOCK; k++)
                    {
                        for (int l = 0; l < BSR_K / WMMA_K_BLOCK; l++)
                        {
                            a_frags_compute[i * SUB_BSR_K_BLOCK + j][k][l][0] = *(uint32_t *)(shmem_a + shmem_offset + lane_id * 8 + 0 + (k * (BSR_K / WMMA_K_BLOCK) + l) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK)) + dep_offset);
                            a_frags_compute[i * SUB_BSR_K_BLOCK + j][k][l][1] = *(uint32_t *)(shmem_a + shmem_offset + lane_id * 8 + 2 + (k * (BSR_K / WMMA_K_BLOCK) + l) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK)) + dep_offset);
                            a_frags_compute[i * SUB_BSR_K_BLOCK + j][k][l][2] = *(uint32_t *)(shmem_a + shmem_offset + lane_id * 8 + 4 + (k * (BSR_K / WMMA_K_BLOCK) + l) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK)) + dep_offset);
                            a_frags_compute[i * SUB_BSR_K_BLOCK + j][k][l][3] = *(uint32_t *)(shmem_a + shmem_offset + lane_id * 8 + 6 + (k * (BSR_K / WMMA_K_BLOCK) + l) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK)) + dep_offset);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < SUB_BSR_M_BLOCK + 1; i++)
            {
                row_ptr_a_local_compute[i] = shmem_a_row_ptr[warp_row * (SUB_BSR_M_BLOCK + 1) + i + warp_dep * (SUB_BSR_M_BLOCK + 1) * NUM_RANK_BLOCK];
            }

            for (int i = 0; i < SUB_BSR_M_BLOCK * SUB_BSR_K_BLOCK; i++)
            {
                col_idx_a_local_compute[i] = shmem_a_col_idx[warp_row * (SUB_BSR_M_BLOCK * SUB_BSR_K_BLOCK) + i + warp_dep * (SUB_BSR_M_BLOCK * SUB_BSR_K_BLOCK) * NUM_RANK_BLOCK];
            }
            ...
            for (int i = 0; i < SUB_BSR_K_BLOCK; i++)
            {
                for (int j = 0; j < SUB_BSR_N_BLOCK; j++)
                {
                    ...
                    for (int k = 0; k < BSR_K / WMMA_K_BLOCK; k++)
                    {
                        for (int l = 0; l < BSR_N / WMMA_N_BLOCK; l++)
                        {
                            b_frags_compute[i * SUB_BSR_N_BLOCK + j][k][l][0] = *(uint32_t *)(shmem_b + shmem_offset + lane_id * 4 + 0 + (k * (BSR_N / WMMA_N_BLOCK) + l) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * ((SUB_K_BLOCK) * (SUB_N_BLOCK)) + dep_offset);
                            b_frags_compute[i * SUB_BSR_N_BLOCK + j][k][l][1] = *(uint32_t *)(shmem_b + shmem_offset + lane_id * 4 + 2 + (k * (BSR_N / WMMA_N_BLOCK) + l) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * ((SUB_K_BLOCK) * (SUB_N_BLOCK)) + dep_offset);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < SUB_BSR_K_BLOCK + 1; i++)
            {
                row_ptr_b_local_compute[i] = shmem_b_row_ptr[warp_col * (SUB_BSR_K_BLOCK + 1) + i + warp_dep * (SUB_BSR_K_BLOCK + 1) * NUM_RANK_BLOCK];
            }

            for (int i = 0; i < SUB_BSR_K_BLOCK * SUB_BSR_N_BLOCK; i++)
            {
                col_idx_b_local_compute[i] = shmem_b_col_idx[warp_col * (SUB_BSR_K_BLOCK * SUB_BSR_N_BLOCK) + i + warp_dep * (SUB_BSR_K_BLOCK * SUB_BSR_N_BLOCK) * NUM_RANK_BLOCK];
            }
    ...
    {
        int nnzC = RowPtrC[BSR_M_BLOCK] * BSR_M * BSR_N;
        int row_start_bsr = warp_row * SUB_BSR_M_BLOCK;
        int row_end_bsr = row_start_bsr + SUB_BSR_M_BLOCK;
        int col_start_bsr = warp_col * SUB_BSR_N_BLOCK;
        int col_end_bsr = col_start_bsr + SUB_BSR_N_BLOCK;
        int offsetC = warp_dep * nnzC;
                ...
                if (col_id >= col_start_bsr && col_id < col_end_bsr)
                {
                    ...
                    for (int i = 0; i < BSR_M / WMMA_M_BLOCK; i++)
                    {
                        for (int j = 0; j < BSR_N / WMMA_N_BLOCK; j++)
                        {
                            ...
                            *(uint32_t *)(ValC + val_offset + (i * BSR_N + lane_row_upper_offset) * BSR_N + lane_col_offset + j * WMMA_N_BLOCK + offsetC) = c_frags[ele_id_local][i][j][0];
                            *(uint32_t *)(ValC + val_offset + (i * BSR_N + lane_row_lower_offset) * BSR_N + lane_col_offset + j * WMMA_N_BLOCK + offsetC) = c_frags[ele_id_local][i][j][1];
                        }
                    }
...
int main(int argc, char *argv[])
{
    ...
    cudaFuncSetAttribute(block_spgemm_3d_half_mma, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size_block_spgemm);
    std::cout << "Launching kernel with " << BLOCKS_PER_GRID << " blocks and "
              << NUM_ALLOC_RANK_BLOCK * WARP_SIZE << " threads per block and " << shmem_size_block_spgemm << " bytes of shared memory" << std::endl;
    ...
    cudaMalloc(&d_ValC, d_nnzC * NUM_RANK_BLOCK * sizeof(half));
    ...
    cudaEventRecord(start);
    block_spgemm_3d_half_mma<<<BLOCKS_PER_GRID, NUM_ALLOC_RANK_BLOCK * WARP_SIZE, shmem_size_block_spgemm>>>(d_RowPtrA, d_ColIdxA, d_ValA, d_RowPtrB, d_ColIdxB, d_ValB, d_RowPtrC, d_ColIdxC, d_ValC, 1.0, 0.0);
    cudaDeviceSynchronize();
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    ...
    std::cout << "[hemeng_log],3d," << nnzCub << "," << M_BLOCK << "," << N_BLOCK << "," << K_BLOCK << "," << NUM_RANK_ALL_BLOCK << "," << tflops << "," << THREADS_PER_BLOCK << std::endl;

    half *h_ValC = (half *)malloc(d_nnzC * NUM_RANK_BLOCK * sizeof(half));
    cudaMemcpy(h_ValC, d_ValC, d_nnzC * NUM_RANK_BLOCK * sizeof(half), cudaMemcpyDeviceToHost);

    for (int i = 0; i < nnzC; i++)
    {
        for (int j = 1; j < NUM_RANK_BLOCK; j++)
        {

            h_ValC[i] += h_ValC[j * nnzC + i];
        }
    }
