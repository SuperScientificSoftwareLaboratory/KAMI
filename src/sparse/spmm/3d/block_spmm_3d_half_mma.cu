...
#define NUMS_COL (BSR_COLS / NUM_RANK_BLOCK / NUM_RANK_BLOCK)

const int shmem_size_block_spmm = ((SUB_M_BLOCK * SUB_K_BLOCK * NUM_RANK_BLOCK * NUM_RANK_BLOCK) + (SUB_K_BLOCK * SUB_N_BLOCK * NUM_RANK_BLOCK * NUM_RANK_BLOCK)) * sizeof(half);
...
__global__ void block_spmm_3d_half_mma(const int *blk_rowptr, const int *blk_colidx, const half *bsr_vals, const half *B, half *C, const half alpha, const half beta)
{
    ...
    {
        int row_start = (warp_row * (BSR_COLS / NUMS_COL) + (warp_col * NUM_RANK_BLOCK + warp_dep)) * NUMS_ROW;
        for (int j = blk_rowptr[row_start + i]; j < blk_rowptr[row_start + i + 1]; ++j)
        {
            ...
            a_frags[i][col][0] = *(uint32_t *)(&(A[global_offset_upper_left]));
            a_frags[i][col][1] = *(uint32_t *)(&(A[global_offset_lower_left]));
            a_frags[i][col][2] = *(uint32_t *)(&(A[global_offset_upper_right]));
            a_frags[i][col][3] = *(uint32_t *)(&(A[global_offset_lower_right]));
        }
    }
        ...
        for (int idx_stage = 0; idx_stage < NUM_RANK_BLOCK; ++idx_stage)
        {
            ...
            if (warp_col == idx_stage)
            {
                for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
                {
                    for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; ++j)
                    {
                        ...
                        *(uint32_t *)(shmem_a + lane_id * 8 + 0 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset) = a_frags[i][j][0];
                        *(uint32_t *)(shmem_a + lane_id * 8 + 2 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset) = a_frags[i][j][1];
                        *(uint32_t *)(shmem_a + lane_id * 8 + 4 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset) = a_frags[i][j][2];
                        *(uint32_t *)(shmem_a + lane_id * 8 + 6 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset) = a_frags[i][j][3];
                    }
                }
            }
            ...
            if (warp_row == idx_stage)
            {
                for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; ++i)
                {
                    for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; ++j)
                    {
                        ...
                        *(uint32_t *)(shmem_b + lane_id * 4 + 0 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_N_BLOCK * WMMA_K_BLOCK + dep_offset + col_offset) = b_frags[i][j][0];
                        *(uint32_t *)(shmem_b + lane_id * 4 + 2 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_N_BLOCK * WMMA_K_BLOCK + dep_offset + col_offset) = b_frags[i][j][1];
                    }
                }
            }
            ...
            for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; ++i)
            {
                for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; ++j)
                {
                    int dep_offset = warp_dep * WMMA_N_BLOCK * WMMA_K_BLOCK;
                    int col_offset = warp_col * SUB_K_BLOCK * SUB_N_BLOCK * NUM_RANK_BLOCK;
                    b_frags_compute[i][j][0] = *(uint32_t *)(shmem_b + lane_id * 4 + 0 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_N_BLOCK * WMMA_K_BLOCK + dep_offset + col_offset);
                    b_frags_compute[i][j][1] = *(uint32_t *)(shmem_b + lane_id * 4 + 2 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_N_BLOCK * WMMA_K_BLOCK + dep_offset + col_offset);
                }
            }

            for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
            {
                int row_start = warp_row * (BSR_COLS / NUMS_COL) * NUMS_ROW + (idx_stage * NUM_RANK_BLOCK + warp_dep) * NUMS_ROW;
                for (int k = blk_rowptr[row_start + i]; k < blk_rowptr[row_start + i + 1]; ++k)
                {
                    ...
                    int dep_offset = warp_dep * WMMA_M_BLOCK * WMMA_K_BLOCK;
                    int row_offset = warp_row * SUB_M_BLOCK * SUB_K_BLOCK * NUM_RANK_BLOCK;
                    a_frags_compute[0] = *(uint32_t *)(shmem_a + lane_id * 8 + 0 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + col) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                    a_frags_compute[1] = *(uint32_t *)(shmem_a + lane_id * 8 + 2 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + col) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                    a_frags_compute[2] = *(uint32_t *)(shmem_a + lane_id * 8 + 4 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + col) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                    a_frags_compute[3] = *(uint32_t *)(shmem_a + lane_id * 8 + 6 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + col) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                    for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; ++j)
                    {
...
int main(int argc, char *argv[])
{
    ...
    cudaFuncSetAttribute(block_spmm_3d_half_mma, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size_block_spmm);

    std::cout << "Launching kernel with " << BLOCKS_PER_GRID << " blocks and "
              << NUM_ALLOC_RANK_BLOCK * WARP_SIZE << " threads per block and " << shmem_size_block_spmm << " bytes of shared memory" << std::endl;

    cudaEventRecord(start);
    block_spmm_3d_half_mma<<<BLOCKS_PER_GRID, NUM_ALLOC_RANK_BLOCK * WARP_SIZE, shmem_size_block_spmm>>>(d_blk_rowptr, d_blk_colidx, d_bsr_val, d_B, d_C, 1.0, 0.0);
