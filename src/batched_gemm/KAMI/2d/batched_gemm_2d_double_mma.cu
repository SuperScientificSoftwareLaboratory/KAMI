...
__global__ void batched_gemm_2d_double_mma(const double *A, const double *B, double *C, const double alpha, const double beta)
{
                ...
                for (int idx_pipe_m = 0; idx_pipe_m < NUM_PIPE_M_BLOCK; idx_pipe_m++)
                {
                    for (int idx_pipe_k = 0; idx_pipe_k < NUM_PIPE_K_BLOCK; idx_pipe_k++)
                    {
                        if (warp_id_col == idx_stage)
                        {
                            for (int i = 0; i < SUB_M_BLOCK / NUM_PIPE_M_BLOCK / WMMA_M_BLOCK; i++)
                            {
                                for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; j++)
                                {
                                    ...
                                    shmem_a[lane_id * 4 + 0 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_id_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))] = a_frags[real_i][real_j][0];
                                    shmem_a[lane_id * 4 + 1 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_id_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))] = a_frags[real_i][real_j][1];
                                    shmem_a[lane_id * 4 + 2 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_id_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))] = a_frags[real_i][real_j][2];
                                    shmem_a[lane_id * 4 + 3 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_id_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))] = a_frags[real_i][real_j][3];
                                }
                            }
                        }
                        ...
                        if (warp_id_row == idx_stage)
                        {
                            for (int i = 0; i < (SUB_N_BLOCK / NUM_PIPE_N_BLOCK) / WMMA_N_BLOCK; ++i)
                            {
                                for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; ++j)
                                {
                                    ...
                                    int real_j = j + idx_pipe_k * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK);
                                    shmem_b[lane_id * 2 + 0 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK + warp_id_col * ((SUB_N_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))] = b_frags[real_i][real_j][0];
                                    shmem_b[lane_id * 2 + 1 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK + warp_id_col * ((SUB_N_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))] = b_frags[real_i][real_j][1];
                                }
                            }
                        }
                        ...
                        for (int i = 0; i < SUB_M_BLOCK / NUM_PIPE_M_BLOCK / WMMA_M_BLOCK; i++)
                        {
                            for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; j++)
                            {
                                a_frags_compute[i][j][0] = shmem_a[lane_id * 4 + 0 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_id_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))];
                                a_frags_compute[i][j][1] = shmem_a[lane_id * 4 + 1 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_id_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))];
                                a_frags_compute[i][j][2] = shmem_a[lane_id * 4 + 2 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_id_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))];
                                a_frags_compute[i][j][3] = shmem_a[lane_id * 4 + 3 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_id_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))];
                            }
                        }
                        ...
                        for (int i = 0; i < (SUB_N_BLOCK / NUM_PIPE_N_BLOCK) / WMMA_N_BLOCK; i++)
                        {
                            for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; j++)
                            {
                                b_frags_compute[i][j][0] = shmem_b[lane_id * 2 + 0 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK + warp_id_col * ((SUB_N_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))];
                                b_frags_compute[i][j][1] = shmem_b[lane_id * 2 + 1 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK + warp_id_col * ((SUB_N_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_K_BLOCK))];
                            }
                        }
...
#pragma omp parallel for
    ...
    cudaFuncSetAttribute(batched_gemm_2d_double_mma, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size_block_gemm);
    ...
    batched_gemm_2d_double_mma<<<NUM_BATCHES, NUM_ALLOC_RANK_BLOCK * WARP_SIZE, shmem_size_block_gemm>>>(d_A, d_B, d_C, 1.0, 0.0);
    ...
    std::cout << "[hemeng_log],2d," << M_BLOCK << "," << N_BLOCK << "," << K_BLOCK << "," << NUM_BATCHES << "," << tflops << "," << THREADS_PER_BLOCK << std::endl;
