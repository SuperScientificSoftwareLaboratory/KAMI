...
#define SUB_REG_K_BLOCK (REG_K_BLOCK / NUM_RANK_BLOCK / NUM_RANK_BLOCK)
#define SUB_SMEM_K_BLOCK (SMEM_K_BLOCK / NUM_RANK_BLOCK / NUM_RANK_BLOCK)

const int shmem_size_communication_block = (SUB_M_BLOCK * SUB_REG_K_BLOCK * NUM_RANK_BLOCK * NUM_RANK_BLOCK) + (SUB_REG_K_BLOCK * SUB_N_BLOCK * NUM_RANK_BLOCK * NUM_RANK_BLOCK);
...
__global__ void block_gemm_3d_half_mma(const half *A, const half *B, half *C, const half alpha, const half beta)
{
    ...
    half *shmem_a = shmem;
    half *shmem_b = shmem + SUB_M_BLOCK * SUB_REG_K_BLOCK * NUM_RANK_BLOCK * NUM_RANK_BLOCK;
    ...
    uint32_t a_frags_compute[SUB_M_BLOCK / WMMA_M_BLOCK][SUB_REG_K_BLOCK / WMMA_K_BLOCK][4];
    uint32_t b_frags_compute[SUB_N_BLOCK / WMMA_N_BLOCK][SUB_REG_K_BLOCK / WMMA_K_BLOCK][2];
    ...
    for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
    {
        for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK; ++j)
        {
            ...
            int block_col = (warp_col * NUM_RANK_BLOCK + warp_dep) * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j;
    ...
    for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; ++i)
    {
        for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK; ++j)
        {
            ...
            int block_col = (warp_row * NUM_RANK_BLOCK + warp_dep) * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j;
    ...
    for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
    {
        ...
        {
            int block_row = warp_row * (SUB_M_BLOCK / WMMA_M_BLOCK) + i;
            int block_col = (warp_col * NUM_RANK_BLOCK + warp_dep) * (SUB_SMEM_K_BLOCK / WMMA_K_BLOCK) + j + REG_K_BLOCK / WMMA_K_BLOCK;
    ...
    for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; ++i)
    {
        ...
        {
            int block_row = warp_col * (SUB_N_BLOCK / WMMA_N_BLOCK) + i;
            int block_col = (warp_row * NUM_RANK_BLOCK + warp_dep) * (SUB_SMEM_K_BLOCK / WMMA_K_BLOCK) + j + REG_K_BLOCK / WMMA_K_BLOCK;
        ...
        for (int idx_stage = 0; idx_stage < NUM_RANK_BLOCK; ++idx_stage)
        {
            if (warp_col == idx_stage)
            {
                for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
                {
                    for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK; ++j)
                    {
                        ...
                        int row_offset = warp_row * SUB_M_BLOCK * SUB_REG_K_BLOCK * NUM_RANK_BLOCK;

                        *(uint32_t *)(shmem_a + lane_id * 8 + 0 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset) = a_frags[i][j][0];
                        *(uint32_t *)(shmem_a + lane_id * 8 + 2 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset) = a_frags[i][j][1];
                        *(uint32_t *)(shmem_a + lane_id * 8 + 4 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset) = a_frags[i][j][2];
                        *(uint32_t *)(shmem_a + lane_id * 8 + 6 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset) = a_frags[i][j][3];
                    }
                }
            }
            ...
            if (warp_row == idx_stage)
            {
                for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; ++i)
                {
                    for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK; ++j)
                    {
                        ...
                        int col_offset = warp_col * SUB_REG_K_BLOCK * SUB_N_BLOCK * NUM_RANK_BLOCK;

                        *(uint32_t *)(shmem_b + lane_id * 4 + 0 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_N_BLOCK * WMMA_K_BLOCK + dep_offset + col_offset) = b_frags[i][j][0];
                        *(uint32_t *)(shmem_b + lane_id * 4 + 2 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_N_BLOCK * WMMA_K_BLOCK + dep_offset + col_offset) = b_frags[i][j][1];
                    }
                }
            }
            ...
            for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
            {
                for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK; ++j)
                {
                    ...
                    a_frags_compute[i][j][0] = *(uint32_t *)(shmem_a + lane_id * 8 + 0 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                    a_frags_compute[i][j][1] = *(uint32_t *)(shmem_a + lane_id * 8 + 2 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                    a_frags_compute[i][j][2] = *(uint32_t *)(shmem_a + lane_id * 8 + 4 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                    a_frags_compute[i][j][3] = *(uint32_t *)(shmem_a + lane_id * 8 + 6 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                }
            }
            ...
            for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; ++i)
            {
                for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK; ++j)
                {
                    ...
                    b_frags_compute[i][j][0] = *(uint32_t *)(shmem_b + lane_id * 4 + 0 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_N_BLOCK * WMMA_K_BLOCK + dep_offset + col_offset);
                    b_frags_compute[i][j][1] = *(uint32_t *)(shmem_b + lane_id * 4 + 2 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_N_BLOCK * WMMA_K_BLOCK + dep_offset + col_offset);
                }
            }
            ...
            for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
            {
                for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; ++j)
                {
                    for (int k = 0; k < SUB_REG_K_BLOCK / WMMA_K_BLOCK; ++k)
                    {
