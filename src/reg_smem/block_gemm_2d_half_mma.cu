...
#define K_SPLIT_BLOCK 2
#define REG_K_SPLIT_BLOCK 1
...
#define SUB_REG_K_BLOCK (REG_K_BLOCK / NUM_RANK_BLOCK)
#define SUB_SMEM_K_BLOCK (SMEM_K_BLOCK / NUM_RANK_BLOCK)

#define NUM_PIPE_K_BLOCK 2

const int shmem_size_communication_block = ((M_BLOCK * (SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK)) + (K_BLOCK / NUM_PIPE_K_BLOCK * SUB_N_BLOCK));
...
__global__ void block_gemm_2d_half_mma(const half *A, const half *B, half *C, const half alpha, const half beta)
{
    ...
    half *shmem_a = shmem;
    half *shmem_b = shmem + M_BLOCK * SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK;
    ...
    uint32_t (*shmem_a_cache)[SUB_SMEM_K_BLOCK / WMMA_K_BLOCK][WMMA_M_BLOCK * WMMA_K_BLOCK / 2] =
        reinterpret_cast<uint32_t (*)[SUB_SMEM_K_BLOCK / WMMA_K_BLOCK][WMMA_M_BLOCK * WMMA_K_BLOCK / 2]>(shmem + shmem_size_communication_block + warp_id * SUB_M_BLOCK * SUB_SMEM_K_BLOCK);
    uint32_t (*shmem_b_cache)[SUB_SMEM_K_BLOCK / WMMA_K_BLOCK][WMMA_N_BLOCK * WMMA_K_BLOCK / 2] =
        reinterpret_cast<uint32_t (*)[SUB_SMEM_K_BLOCK / WMMA_K_BLOCK][WMMA_N_BLOCK * WMMA_K_BLOCK / 2]>(shmem + shmem_size_communication_block + shmem_size_cache_a_block + warp_id * SUB_N_BLOCK * SUB_SMEM_K_BLOCK);

    uint32_t a_frags[SUB_M_BLOCK / WMMA_M_BLOCK][SUB_REG_K_BLOCK / WMMA_K_BLOCK][4];
    uint32_t b_frags[SUB_N_BLOCK / WMMA_N_BLOCK][SUB_REG_K_BLOCK / WMMA_K_BLOCK][2];
    uint32_t a_frags_compute[SUB_M_BLOCK / WMMA_M_BLOCK][SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK][4];
    uint32_t b_frags_compute[SUB_N_BLOCK / WMMA_N_BLOCK][SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK][2];
    uint32_t c_frags[SUB_M_BLOCK / WMMA_M_BLOCK][SUB_N_BLOCK / WMMA_N_BLOCK][2];

    for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
    {
        for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK; ++j)
        {
            int block_row = warp_row * (SUB_M_BLOCK / WMMA_M_BLOCK) + i;
            int block_col = warp_col * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j;
    ...
    for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; ++i)
    {
        for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK; ++j)
        {
            ...
            int block_col = warp_row * (SUB_REG_K_BLOCK / WMMA_K_BLOCK) + j;
    ...
    for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
    {
        for (int j = 0; j < SUB_SMEM_K_BLOCK / WMMA_K_BLOCK; ++j)
        {
            int block_row = warp_row * (SUB_M_BLOCK / WMMA_M_BLOCK) + i;
            int block_col = warp_col * (SUB_SMEM_K_BLOCK / WMMA_K_BLOCK) + j + REG_K_BLOCK / WMMA_K_BLOCK;
            int real_row_upper = block_row * WMMA_M_BLOCK + (lane_id >> 2);
            int real_row_lower = real_row_upper + 8;
            int real_col_left = block_col * WMMA_K_BLOCK + (lane_id % 4) * 2;
            int real_col_right = real_col_left + 8;
    ...
    for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; ++i)
    {
        ...
        {
            int block_row = warp_col * (SUB_N_BLOCK / WMMA_N_BLOCK) + i;
            int block_col = warp_row * (SUB_SMEM_K_BLOCK / WMMA_K_BLOCK) + j + REG_K_BLOCK / WMMA_K_BLOCK;
            int real_row = block_row * WMMA_N_BLOCK + (lane_id >> 2);
            int real_col_left = block_col * WMMA_K_BLOCK + (lane_id % 4) * 2;
            int real_col_right = real_col_left + 8;
            int global_offset_left = real_row * K_BLOCK + real_col_left;
            ...
            // if (threadIdx.x == 0)
            // {
            //     // printf("shmem_b_cache[%d][%d][%d]: %f, B[%d]: %f\n", i, j, lane_id * 2 + 0, (float)(*(half *)(&(shmem_b_cache[i][j][lane_id * 2 + 0]))), global_offset_left, __half2float(B[global_offset_left]));
            //     // printf("shmem_b_cache[%d][%d][%d]: %f, B[%d]: %f\n", i, j, lane_id * 2 + 1, (float)(*(half *)(&(shmem_b_cache[i][j][lane_id * 2 + 1]))), global_offset_right, __half2float(B[global_offset_right]));

            //     printf("[%d]shmem_b_cache[%d][%d][%d]: %f, B[%d]: %f\n", threadIdx.x, i, j, lane_id * 2 + 0, (float)(*(half *)(&(shmem_b_cache[i][j][lane_id * 2 + 0]))), global_offset_left, __half2float(B[global_offset_left]));
            //     printf("[%d]shmem_b_cache[%d][%d][%d]: %f, B[%d]: %f\n", threadIdx.x, i, j, lane_id * 2 + 1, (float)(*(half *)(&(shmem_b_cache[i][j][lane_id * 2 + 1]))), global_offset_right, __half2float(B[global_offset_right]));
            // }
        }
    }
    ...
    for (int idx_iter = 0; idx_iter < NUM_ITER; ++idx_iter)
    {
        for (int idx_stage = 0; idx_stage < NUM_RANK_BLOCK; ++idx_stage)
        {
            for (int idx_pipe = 0; idx_pipe < NUM_PIPE_K_BLOCK; ++idx_pipe)
            {
                if (warp_col == idx_stage)
                {
                    for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; i++)
                    {
                        for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; j++)
                        {
                            int real_j = j + idx_pipe * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK);
                            *(uint32_t *)(shmem_a + lane_id * 8 + 0 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK))) = a_frags[i][real_j][0];
                            *(uint32_t *)(shmem_a + lane_id * 8 + 2 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK))) = a_frags[i][real_j][1];
                            *(uint32_t *)(shmem_a + lane_id * 8 + 4 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK))) = a_frags[i][real_j][2];
                            *(uint32_t *)(shmem_a + lane_id * 8 + 6 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK))) = a_frags[i][real_j][3];
                        }
                    }
                }
                ...
                if (warp_row == idx_stage)
                {
                    for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; i++)
                    {
                        ...
                        {
                            int real_j = j + idx_pipe * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK);
                            *(uint32_t *)(shmem_b + lane_id * 4 + 0 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * ((SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK) * (SUB_N_BLOCK))) = b_frags[i][real_j][0];
                            *(uint32_t *)(shmem_b + lane_id * 4 + 2 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * ((SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK) * (SUB_N_BLOCK))) = b_frags[i][real_j][1];
                        }
                    }
                }
                ...
                for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; i++)
                {
                    for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; j++)
                    {
                        a_frags_compute[i][j][0] = *(uint32_t *)(shmem_a + lane_id * 8 + 0 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK)));
                        a_frags_compute[i][j][1] = *(uint32_t *)(shmem_a + lane_id * 8 + 2 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK)));
                        a_frags_compute[i][j][2] = *(uint32_t *)(shmem_a + lane_id * 8 + 4 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK)));
                        a_frags_compute[i][j][3] = *(uint32_t *)(shmem_a + lane_id * 8 + 6 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK)));
                    }
                }
                ...
                for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; i++)
                {
                    for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; j++)
                    {
                        b_frags_compute[i][j][0] = *(uint32_t *)(shmem_b + lane_id * 4 + 0 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * ((SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK) * (SUB_N_BLOCK)));
                        b_frags_compute[i][j][1] = *(uint32_t *)(shmem_b + lane_id * 4 + 2 + (i * (SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK) + j) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * ((SUB_REG_K_BLOCK / NUM_PIPE_K_BLOCK) * (SUB_N_BLOCK)));
                    }
                }
                ...
                for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
                {
                    for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; ++j)
                    {
                        for (int k = 0; k < SUB_REG_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK; ++k)
                        {
        ...
        for (int idx_s2r_stage = 0; idx_s2r_stage < SUB_SMEM_K_BLOCK / SUB_REG_K_BLOCK; idx_s2r_stage++)
        {
            int s2r_k_offset = idx_s2r_stage * (SUB_REG_K_BLOCK / WMMA_K_BLOCK);
            ...
            for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; i++)
            {
                for (int j = 0; j < SUB_REG_K_BLOCK / WMMA_K_BLOCK; j++)
                {
                    ...
                    // if (threadIdx.x == 0)
                    // {
                    //     printf("[%d]b_frags[%d][%d][0]: %f, shmem_b_cache[i][s2r_k_offset + j][0 + lane_id * 2]: %f\n", threadIdx.x, i, j, (float)(*(half *)(&(b_frags[i][j][0]))), (float)(*(half *)(&(shmem_b_cache[i][s2r_k_offset + j][0 + lane_id * 2]))));
                    //     printf("[%d]b_frags[%d][%d][1]: %f, shmem_b_cache[i][s2r_k_offset + j][1 + lane_id * 2]: %f\n", threadIdx.x, i, j, (float)(*(half *)(&(b_frags[i][j][1]))), (float)(*(half *)(&(shmem_b_cache[i][s2r_k_offset + j][1 + lane_id * 2]))));
                    // }
                }
            }
