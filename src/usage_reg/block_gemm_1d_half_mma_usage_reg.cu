            ...
            for (int idx_pipe = 0; idx_pipe < NUM_PIPE_BLOCK; ++idx_pipe)
            {
                ...
                if (warp_id == idx_stage)
                {
                    for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; i++)
                    {
                        for (int j = 0; j < K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK; j++)
                        {
                            ...
                            *(uint32_t *)(shmem_b + lane_id * 4 + (i * (K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK) = b_frags[i][real_j][0];
                            *(uint32_t *)(shmem_b + lane_id * 4 + (i * (K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK + 2) = b_frags[i][real_j][1];
                        }
                    }
                }
                ...
                for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; i++)
                {
                    for (int j = 0; j < K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK; j++)
                    {
                        ...
                        b_frags_compute[i][j][0] = *(uint32_t *)(shmem_b + lane_id * 4 + (i * (K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK);
                        b_frags_compute[i][j][1] = *(uint32_t *)(shmem_b + lane_id * 4 + (i * (K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK + 2);
                    }
                }
