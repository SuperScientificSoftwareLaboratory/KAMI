        ...
        for (int idx_stage = 0; idx_stage < NUM_RANK_BLOCK; ++idx_stage)
        {
            ...
            for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
            {
                for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; ++j)
                {
                    ...
                    a_frags_compute[i][j][0] = *(uint32_t *)(shmem_a + lane_id * 8 + 0 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                    a_frags_compute[i][j][1] = *(uint32_t *)(shmem_a + lane_id * 8 + 2 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                    a_frags_compute[i][j][2] = *(uint32_t *)(shmem_a + lane_id * 8 + 4 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                    a_frags_compute[i][j][3] = *(uint32_t *)(shmem_a + lane_id * 8 + 6 + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * NUM_RANK_BLOCK * WMMA_M_BLOCK * WMMA_K_BLOCK + dep_offset + row_offset);
                }
            }
