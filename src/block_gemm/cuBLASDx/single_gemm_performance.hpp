...
int benchmark_mixed_precision_gemm(const cudaStream_t& stream, bool verbose = false) {
    ...
    static constexpr unsigned int kernel_warm_up_repeats = 0;
    printf("inside_repeats: %d, kernel_repeats: %d, kernel_warm_up_repeats: %d\n", inside_repeats, kernel_repeats, kernel_warm_up_repeats);
    ...
    double ms_n1 = example::measure::execution(
        [&](cudaStream_t stream)
        {
            gemm_kernel<gemm_type><<<GRID_SIZE, gemm_type::block_dim, gemm_type::shared_memory_size, stream>>>(
