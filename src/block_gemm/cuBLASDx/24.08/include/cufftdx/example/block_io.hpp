    ...
    struct io {
        ...
        static inline __device__ unsigned int input_batch_offset(unsigned int local_fft_id) {
            ...
            unsigned int global_fft_id =
                FFT::ffts_per_block == 1 ? blockIdx.x : (blockIdx.x * (FFT::ffts_per_block / FFT::implicit_type_batching) + local_fft_id);
                ...
                if ((i * stride + threadIdx.x) < FFT::input_length) {
                    if constexpr (needs_half2_format_conversion) {
                        reinterpret_cast<input_t*>(thread_data)[i] =
                            __io::convert_to_rrii<InputInRRIILayout>(reinterpret_cast<const input_t*>(input)[index]);
                    } else {
                        reinterpret_cast<input_t*>(thread_data)[i] = reinterpret_cast<const input_t*>(input)[index];
                    }
                    index += stride;
                }
            }
                ...
                if ((i * stride + threadIdx.x) < FFT::output_length) {
                    if constexpr (needs_half2_format_conversion) {
                        reinterpret_cast<output_t*>(output)[index] =
                            __io::convert_to_riri<OutputInRRIILayout>(reinterpret_cast<const output_t*>(thread_data)[i]);
                    } else {
                        reinterpret_cast<output_t*>(output)[index] = reinterpret_cast<const output_t*>(thread_data)[i];
                    }
                    index += stride;
                }
            }
