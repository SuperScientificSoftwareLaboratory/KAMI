...
__launch_bounds__(FFTX::max_threads_per_block) __global__
    ...
    if (blockIdx.x < (cufftdx::size_of<FFTY>::value / FFTY::ffts_per_block)) {
        ...
        if constexpr (UseSharedMemoryStridedIO) {
            example::io_strided<FFTX>::load_strided<stride>(output, thread_data, reinterpret_cast<InputOutputType*>(shared_mem), local_fft_id);
        } else {
        ...
        // precision is used
        if constexpr (UseSharedMemoryStridedIO) {
            example::io_strided<FFTX>::store_strided<stride>(thread_data,
                                                             reinterpret_cast<InputOutputType*>(shared_mem),
                                                             output,
                                                             local_fft_id);
        } else {
...
struct mixed_fft_2d_functor {
    void operator()() { return mixed_fft_2d<Arch>(); }
};
