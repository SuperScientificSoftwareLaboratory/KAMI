...
const int shmem_size_block_gemm = ((M_BLOCK * (SUB_K_BLOCK)) + (K_BLOCK * SUB_N_BLOCK)) * sizeof(half);
...
__global__ void block_gemm_2d_half_wmma(const half *A, const half *B, half *C, const half alpha, const half beta)
{
    ...
    for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
    {
        for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; ++j)
        {
            ...
            int real_col = warp_col * (SUB_K_BLOCK / WMMA_K_BLOCK) + j;
    ...
    for (int i = 0; i < SUB_K_BLOCK / WMMA_K_BLOCK; ++i)
    {
        ...
        {
            int real_row = warp_row * (SUB_K_BLOCK / WMMA_K_BLOCK) + i;
        ...
        for (int idx_stage = 0; idx_stage < NUM_RANK_BLOCK; ++idx_stage)
        {
            ...
            if (warp_col == idx_stage)
            {
                for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; i++)
                {
                    for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; j++)
                    {
                        for (int k = 0; k < 8; k++)
                        {
                            shmem_a[lane_id * 8 + k + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK))] = a_frags[i][j].x[k];
                        }
                    }
                }
            }
            ...
            if (warp_row == idx_stage)
            {
                for (int i = 0; i < SUB_K_BLOCK / WMMA_K_BLOCK; i++)
                {
                    for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; j++)
                    {
                        for (int k = 0; k < 8; k++)
                        {
                            shmem_b[lane_id * 8 + k + (i * (SUB_N_BLOCK / WMMA_N_BLOCK) + j) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * ((SUB_K_BLOCK) * (SUB_N_BLOCK))] = b_frags[i][j].x[k];
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; i++)
            {
                for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; j++)
                {
                    for (int k = 0; k < 8; k++)
                    {
                        a_frags_compute[i][j].x[k] = shmem_a[lane_id * 8 + k + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK))];
                    }
                }
            }
            ...
            for (int i = 0; i < SUB_K_BLOCK / WMMA_K_BLOCK; i++)
            {
                for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; j++)
                {
                    for (int k = 0; k < 8; k++)
                    {
                        b_frags_compute[i][j].x[k] = shmem_b[lane_id * 8 + k + (i * (SUB_N_BLOCK / WMMA_N_BLOCK) + j) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * ((SUB_K_BLOCK) * (SUB_N_BLOCK))];
                    }
                }
            }
    ...
    for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; i++)
    {
        for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; j++)
        {
            ...
            rocwmma::store_matrix_sync(C + real_idx, c_frags[i][j], N_BLOCK, rocwmma::mem_row_major);
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    hipFuncSetAttribute(reinterpret_cast<const void *>(block_gemm_2d_half_wmma), hipFuncAttributeMaxDynamicSharedMemorySize, shmem_size_block_gemm);
    ...
    block_gemm_2d_half_wmma<<<BLOCKS_PER_GRID, NUM_ALLOC_RANK_BLOCK * WARP_SIZE, shmem_size_block_gemm>>>(d_A, d_B, d_C, 1.0, 0.0);
    hipEventRecord(stop);
    ...
    std::cout << "[hemeng_log],2d," << M_BLOCK << "," << N_BLOCK << "," << K_BLOCK << "," << NUM_RANK_BLOCK << "," << NUM_ALLOC_RANK_BLOCK << "," << tflops << "," << NUM_ALLOC_RANK_BLOCK * WARP_SIZE << std::endl;
