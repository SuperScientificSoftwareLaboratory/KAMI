...
const int shmem_size_block_gemm = ((M_BLOCK * (SUB_K_BLOCK / NUM_PIPE_BLOCK)) + (K_BLOCK / NUM_PIPE_BLOCK * SUB_N_BLOCK)) * sizeof(float) / NUM_ELE_PER_REG;
...
__global__ void block_gemm_2d_e5m2_mma(const uint8_t *A, const uint8_t *B, float *C, const float alpha, const float beta)
{
    extern __shared__ __align__(16) uint8_t shmem[];

    uint8_t *shmem_a = shmem;
    uint8_t *shmem_b = shmem + M_BLOCK * SUB_K_BLOCK / NUM_PIPE_BLOCK;
    ...
    uint32_t (*a_compute)[SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK][4 * WARP_SIZE] = reinterpret_cast<uint32_t (*)[SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK][4 * WARP_SIZE]>(shmem_a + warp_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK / NUM_PIPE_BLOCK)));
    uint32_t (*b_compute)[SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK][2 * WARP_SIZE] = reinterpret_cast<uint32_t (*)[SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK][2 * WARP_SIZE]>(shmem_b + warp_col * ((SUB_K_BLOCK / NUM_PIPE_BLOCK) * (SUB_N_BLOCK)));
    ...
    for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
    {
        for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; ++j)
        {
            ...
            int real_col_left = block_col * WMMA_K_BLOCK + (lane_id % 4) * 4;
            int real_col_right = real_col_left + 16;
            ...
            for (int idx_pipe = 0; idx_pipe < NUM_PIPE_BLOCK; ++idx_pipe)
            {
                if (warp_col == idx_stage)
                {
                    for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; i++)
                    {
                        for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK; j++)
                        {
                            ...
                            a_compute[i][j][0 + lane_id * 4] = a_frags[i][real_j][0];
                            a_compute[i][j][1 + lane_id * 4] = a_frags[i][real_j][1];
                            a_compute[i][j][2 + lane_id * 4] = a_frags[i][real_j][2];
                            a_compute[i][j][3 + lane_id * 4] = a_frags[i][real_j][3];
                        }
                    }
                }
                ...
                if (warp_row == idx_stage)
                {
                    for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; i++)
                    {
                        for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_BLOCK; j++)
                        {
                            ...
                            b_compute[i][j][0 + lane_id * 2] = b_frags[i][real_j][0];
                            b_compute[i][j][1 + lane_id * 2] = b_frags[i][real_j][1];
                        }
                    }
                }
                ...
                for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
                {
                    for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; ++j)
                    {
                        ...
                        {
                            mma_m16n8k32_e5m2_fp8_fp32acc(c_frags[i][j], &a_compute[i][k][lane_id * 4], &b_compute[j][k][lane_id * 2]);
                        }
                    }
                }
            }
...
int main(int argc, char *argv[])
{
    ...
    cudaFuncSetAttribute(block_gemm_2d_e5m2_mma, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size_block_gemm);
    ...
    block_gemm_2d_e5m2_mma<<<blocksPerGrid, threadsPerBlock, shmem_size_block_gemm>>>(d_A, d_B, d_C, 1.0, 0.0);
