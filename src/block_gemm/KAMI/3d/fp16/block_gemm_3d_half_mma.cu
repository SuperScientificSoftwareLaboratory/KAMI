...
__global__ void block_gemm_3d_half_mma(const half *A, const half *B, half *C, const half alpha, const half beta)
{
    ...
    int dep_offset_a = warp_dep * SUB_M_BLOCK * SUB_K_BLOCK;
    int row_offset_a = warp_row * SUB_M_BLOCK * SUB_K_BLOCK * NUM_RANK_BLOCK;
    uint32_t (*a_compute)[SUB_K_BLOCK / WMMA_K_BLOCK][4 * WARP_SIZE] = reinterpret_cast<uint32_t (*)[SUB_K_BLOCK / WMMA_K_BLOCK][4 * WARP_SIZE]>(shmem_a + dep_offset_a + row_offset_a);

    int dep_offset_b = warp_dep * SUB_K_BLOCK * SUB_N_BLOCK;
    int col_offset_b = warp_col * SUB_K_BLOCK * SUB_N_BLOCK * NUM_RANK_BLOCK;
    uint32_t (*b_compute)[SUB_K_BLOCK / WMMA_K_BLOCK][2 * WARP_SIZE] = reinterpret_cast<uint32_t (*)[SUB_K_BLOCK / WMMA_K_BLOCK][2 * WARP_SIZE]>(shmem_b + dep_offset_b + col_offset_b);
            ...
            if (warp_col == idx_stage)
            {
                for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
                {
                    for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; ++j)
                    {
                        ...
                        a_compute[i][j][lane_id * 4 + 0] = a_frags[i][j][0];
                        a_compute[i][j][lane_id * 4 + 1] = a_frags[i][j][1];
                        a_compute[i][j][lane_id * 4 + 2] = a_frags[i][j][2];
                        a_compute[i][j][lane_id * 4 + 3] = a_frags[i][j][3];
                    }
                }
            }
            ...
            if (warp_row == idx_stage)
            {
                for (int i = 0; i < SUB_N_BLOCK / WMMA_N_BLOCK; ++i)
                {
                    for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; ++j)
                    {
                        ...
                        b_compute[i][j][lane_id * 2 + 0] = b_frags[i][j][0];
                        b_compute[i][j][lane_id * 2 + 1] = b_frags[i][j][1];
                    }
                }
            }
            ...
            for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; ++i)
            {
                for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; ++j)
                {
                    for (int k = 0; k < SUB_K_BLOCK / WMMA_K_BLOCK; ++k)
                    {
                        ...
                        mma_m16n8k16_fp16(c_frags[i][j], &a_compute[i][k][lane_id * 4], &b_compute[j][k][lane_id * 2]);
                    }
                }
            }
