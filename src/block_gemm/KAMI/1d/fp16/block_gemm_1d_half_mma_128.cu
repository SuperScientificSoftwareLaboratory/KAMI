...
#define NUM_PIPE_K_BLOCK 4
#endif

const int shmem_size_block_gemm = (SUB_N_BLOCK) * (K_BLOCK / NUM_PIPE_K_BLOCK) * sizeof(half);
...
{
    extern __shared__ half __align__((SUB_N_BLOCK) * (K_BLOCK / NUM_PIPE_K_BLOCK)) shmem_b[];
    ...
    for (int idx_iter = 0; idx_iter < NUM_ITER; ++idx_iter)
    {
        ...
        {
            for (int idx_k_pipe = 0; idx_k_pipe < NUM_PIPE_K_BLOCK; ++idx_k_pipe)
            {
...
#pragma unroll
                        for (int j = 0; j < K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK / 2; j++)
                        {
                            ...
                            b_compute[i][j][lane_id * 4 + 0] = b_frags[i][real_j_pipe][0];
...
#pragma unroll
                        for (int k = 0; k < K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK / 2; k++)
                        {
                            ...
                            mma_m16n8k16_fp16(c_frags[i][j + idx_stage * (SUB_N_BLOCK / WMMA_N_BLOCK)], a_frags[i][real_k_pipe], &b_compute[j][k][lane_id * 4]);
                            mma_m16n8k16_fp16(c_frags[i][j + idx_stage * (SUB_N_BLOCK / WMMA_N_BLOCK)], a_frags[i][real_k_pipe + 1], &b_compute[j][k][lane_id * 4 + 2]);
                        }
                    }
                }
