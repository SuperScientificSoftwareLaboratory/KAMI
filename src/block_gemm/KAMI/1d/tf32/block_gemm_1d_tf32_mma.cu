...
const int shmem_size_block_gemm = (SUB_N_BLOCK / NUM_PIPE_N_BLOCK) * (K_BLOCK / NUM_PIPE_K_BLOCK) * sizeof(float);
...
__global__ void block_gemm_1d_tf32_mma(const float *A, const float *B, float *C, const float alpha, const float beta)
{
    extern __shared__ float shmem_b[];
    ...
    float a_frags[SUB_M_BLOCK / WMMA_M_BLOCK][K_BLOCK / WMMA_K_BLOCK][4];
    float b_frags[SUB_N_BLOCK / WMMA_N_BLOCK][K_BLOCK / WMMA_K_BLOCK][2];

    float (*b_compute)[K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK][2 * WARP_SIZE] = reinterpret_cast<float (*)[K_BLOCK / WMMA_K_BLOCK / NUM_PIPE_K_BLOCK][2 * WARP_SIZE]>(shmem_b);

    float c_frags[SUB_M_BLOCK / WMMA_M_BLOCK][N_BLOCK / WMMA_N_BLOCK][4];
...
int main(int argc, char *argv[])
{
    ...
    cudaFuncSetAttribute(block_gemm_1d_tf32_mma, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size_block_gemm);
    ...
    block_gemm_1d_tf32_mma<<<BLOCKS_PER_GRID, NUM_ALLOC_RANK_BLOCK * WARP_SIZE, shmem_size_block_gemm>>>(d_A, d_B, d_C, 1.0f, 0.0f);
