...
const int shmem_size_block_gemm = ((M_BLOCK * (SUB_K_BLOCK)) + (K_BLOCK * SUB_N_BLOCK));
...
void block_gemm_2d_half_sycl(const sycl::half *A, const sycl::half *B,
                             ...
                             int *dpct_warpsize)
{
    ...
    auto shmem_b = (sycl::half *)dpct_local + M_BLOCK * SUB_K_BLOCK;
        ...
        for (int idx_stage = 0; idx_stage < NUM_RANK_BLOCK; ++idx_stage)
        {
            ...
            if (warp_col == idx_stage)
            {
                for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; i++)
                {
                    for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; j++)
                    {
                        ...
                        {
                            shmem_a[lane_id * (WMMA_K_BLOCK * WMMA_M_BLOCK / WARP_SIZE) + k + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK))] = a_frags[i][j].x[k];
                        }
                    }
                }
            }
            ...
            if (warp_row == idx_stage)
            {
                for (int i = 0; i < SUB_K_BLOCK / WMMA_K_BLOCK; i++)
                {
                    for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; j++)
                    {
                        ...
                        {
                            shmem_b[lane_id * (WMMA_K_BLOCK * WMMA_N_BLOCK / WARP_SIZE) + k + (i * (SUB_N_BLOCK / WMMA_N_BLOCK) + j) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * ((SUB_K_BLOCK) * (SUB_N_BLOCK))] = b_frags[i][j].x[k];
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < SUB_M_BLOCK / WMMA_M_BLOCK; i++)
            {
                for (int j = 0; j < SUB_K_BLOCK / WMMA_K_BLOCK; j++)
                {
                    ...
                    {
                        a_frags_compute[i][j].x[k] = shmem_a[lane_id * (WMMA_M_BLOCK * WMMA_K_BLOCK / WARP_SIZE) + k + (i * (SUB_K_BLOCK / WMMA_K_BLOCK) + j) * WMMA_M_BLOCK * WMMA_K_BLOCK + warp_row * ((SUB_M_BLOCK) * (SUB_K_BLOCK))];
                    }
                }
            }
            ...
            for (int i = 0; i < SUB_K_BLOCK / WMMA_K_BLOCK; i++)
            {
                for (int j = 0; j < SUB_N_BLOCK / WMMA_N_BLOCK; j++)
                {
                    ...
                    {
                        b_frags_compute[i][j].x[k] = shmem_b[lane_id * (WMMA_N_BLOCK * WMMA_K_BLOCK / WARP_SIZE) + k + (i * (SUB_N_BLOCK / WMMA_N_BLOCK) + j) * WMMA_K_BLOCK * WMMA_N_BLOCK + warp_col * ((SUB_K_BLOCK) * (SUB_N_BLOCK))];
                    }
                }
            }
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "[hemeng_log],2d," << M_BLOCK << "," << N_BLOCK << "," << K_BLOCK << "," << NUM_RANK_BLOCK << "," << milliseconds << "," << tflops << "," << THREADS_PER_BLOCK << std::endl;
