...
const int shmem_size_block_gemm = SUB_K_BLOCK * N_BLOCK;

#define NUM_ITER 1000

#define THREADS_PER_BLOCK (WARP_SIZE * NUM_RANK_BLOCK)

#define BLOCKS_PER_GRID 1000

#define FETCH_INT4(pointer) (reinterpret_cast<double *>(&(pointer))[0])

void block_gemm_1d_half_sycl(const sycl::half *A, const sycl::half *B,
                             ...
                             int *dpct_warpsize)
{
    auto shmem_b = (sycl::half *)dpct_local;
    ...
    *dpct_warpsize = item_ct1.get_sub_group().get_local_range().get(0);
    ...
    dpct::experimental::matrix::joint_matrix<
        ...
        a_frags[SUB_M_BLOCK / WMMA_M_BLOCK][K_BLOCK / WMMA_K_BLOCK];

    dpct::experimental::matrix::joint_matrix<
        dpct::experimental::matrix::b, WMMA_M_BLOCK, WMMA_N_BLOCK, WMMA_K_BLOCK,
        sycl::half, dpct::experimental::matrix::row_major>
        b_frags[SUB_K_BLOCK / WMMA_K_BLOCK][N_BLOCK / WMMA_N_BLOCK];

    dpct::experimental::matrix::joint_matrix<
        dpct::experimental::matrix::b, WMMA_M_BLOCK, WMMA_N_BLOCK, WMMA_K_BLOCK,
        sycl::half, dpct::experimental::matrix::row_major>
        b_frags_compute[SUB_K_BLOCK / WMMA_K_BLOCK][N_BLOCK / WMMA_N_BLOCK];

    dpct::experimental::matrix::joint_matrix<
        dpct::experimental::matrix::accumulator, WMMA_M_BLOCK, WMMA_N_BLOCK,
        WMMA_K_BLOCK, float>
        c_frags[SUB_M_BLOCK / WMMA_M_BLOCK][N_BLOCK / WMMA_N_BLOCK];
        ...
        for (int j = 0; j < K_BLOCK / WMMA_K_BLOCK; ++j)
        {
            sycl::ext::oneapi::experimental::matrix::joint_matrix_load(
                ...
                sycl::address_space_cast<
                    ...
                    A + warp_id * (SUB_M_BLOCK * K_BLOCK) +
                    i * WMMA_M_BLOCK * K_BLOCK + j * WMMA_K_BLOCK),
                K_BLOCK);
        }
    }
        ...
        for (int j = 0; j < N_BLOCK / WMMA_N_BLOCK; ++j)
        {
            sycl::ext::oneapi::experimental::matrix::joint_matrix_load(
                ...
                sycl::address_space_cast<
                    sycl::access::address_space::generic_space,
                    sycl::access::decorated::no, const sycl::half>(
                    B + warp_id * (SUB_K_BLOCK * N_BLOCK) +
                    i * WMMA_K_BLOCK * N_BLOCK + j * WMMA_N_BLOCK),
                N_BLOCK);
        }
    }
...
#pragma unroll
                        ...
                        {
                            shmem_b[lane_id * (WMMA_K_BLOCK * WMMA_N_BLOCK / WARP_SIZE) + k + (i * (N_BLOCK / WMMA_N_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK] = b_frags[i][j].x[k];
                        }
                    }
                }
            }
...
#pragma unroll
                    ...
                    {
                        b_frags_compute[i][j].x[k] = shmem_b[lane_id * (WMMA_N_BLOCK * WMMA_K_BLOCK / WARP_SIZE) + k + (i * (N_BLOCK / WMMA_N_BLOCK) + j) * WMMA_N_BLOCK * WMMA_K_BLOCK];
                    }
                }
            }
                    ...
                    for (int k = 0; k < SUB_K_BLOCK / WMMA_K_BLOCK; ++k)
                    {
                        sycl::ext::oneapi::experimental::matrix::
                            joint_matrix_mad(
                                ...
                                c_frags[i][j].get(),
                                a_frags[i][k + idx_stage * (SUB_K_BLOCK / WMMA_K_BLOCK)].get(),
        ...
        for (int j = 0; j < N_BLOCK / WMMA_N_BLOCK; j++)
        {
            sycl::ext::oneapi::experimental::matrix::joint_matrix_store(
                ...
                sycl::address_space_cast<
                    ...
                    C + warp_id * (SUB_M_BLOCK * N_BLOCK) +
                    i * WMMA_M_BLOCK * N_BLOCK + j * WMMA_N_BLOCK),
                N_BLOCK,
...
int main(int argc, char *argv[])
{
    ...
    d_B = sycl::malloc_device<sycl::half>(K_BLOCK * N_BLOCK,
                                          dpct::get_in_order_queue());
    d_C = sycl::malloc_device<float>(M_BLOCK * N_BLOCK,
                                     dpct::get_in_order_queue());
    ...
    dpct::get_in_order_queue()
        ...
        .wait();
    dpct::get_in_order_queue()
        .memset(d_C, 0, M_BLOCK * N_BLOCK * sizeof(float))
        .wait();
    ...
    std::cout << "[hemeng_log],1d," << M_BLOCK << "," << N_BLOCK << "," << K_BLOCK << "," << NUM_RANK_BLOCK << "," << milliseconds << "," << tflops << "," << THREADS_PER_BLOCK << std::endl;

    float *h_C =
        (float *)malloc(M_BLOCK * N_BLOCK * sizeof(float));
    dpct::get_in_order_queue()
        .memcpy(h_C, d_C, M_BLOCK * N_BLOCK * sizeof(float))
        .wait();
